...
function docsOptions(environment) {
    return {
        ...
        beforeDefaultRemarkPlugins: [
            ...
            // A fold, from a directive rather than from raw HTML - see the plugin.
            require('./plugins/remark-details'),
            // A table whose first cell spans rows and whose cells may hold folds - see the plugin.
            require('./plugins/remark-grouped-table'),
            // A note raised above the line, such as the pact beside a caller - see the plugin.
            require('./plugins/remark-superscript'),
        ],
...
/** The version the footer names after the service, where the build could read it. */
const service = site.serviceVersion ? ` ${site.serviceVersion}` : '';
...
const config = {
    ...
    // All the JavaScript of this site lives in the template rather than in generated content. The first two fill
    // in the page describing the documentation, what the run cost and the live status; the third makes every
    // table of every page sortable, and filterable above 15 rows.
    clientModules: [
        require.resolve('./src/clientModules/publicationNumbers.js'),
        require.resolve('./src/clientModules/liveStatus.js'),
        require.resolve('./src/clientModules/tableControls.js'),
    ],
    themeConfig:
    /** @type {import('@docusaurus/preset-classic').ThemeConfig} */
        ({
            ...
            docs: {
                sidebar: {
                    hideable: true,
                    // The sidebar shows the path to the open page and little else: every generated category
                    // starts closed, and opening one closes its siblings.
            ...
            footer: {
                ...
                // format has one definition rather than one here and one on every generated page. The version
                // is left out where the build could not read it.
                copyright: site.generatedAtDisplay
                    ? `Generated by the jEAP Doc Service${service} on ${site.generatedAtDisplay}.`
                    : `Generated by the jEAP Doc Service${service}.`,
            },
            prism: {
