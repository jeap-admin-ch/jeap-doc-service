...
| `jeap.doc.generator.max-context-components`                        | `40`              | How many **sibling components** a component's context view may draw. Only the siblings a component actually exchanges something with are candidates, so this is reached far less often than the number of components of a system would suggest. Above it the diagram is cut, the page says so, and its table of relations lists every one                                                                                                    |
| `jeap.doc.generator.max-schema-table-diagram`                      | `100`             | How many entries an **entity relationship diagram** may draw. The limit is the reader's browser rather than the page: the engine lays a diagram out by recursion, so a schema of three hundred tables is not a large picture but no picture at all. Above this the page says how many tables were left out - and the list of tables carries them - up to its own, larger bound below. **An entry is one table, or one family of partitions**: a partitioned table is grouped into a single entry before either bound applies                                   |
| `jeap.doc.generator.max-schema-table-list`                         | `200`             | How many entries a **database schema page** writes with their columns, by name. **The one bound on the facts rather than on a picture**, and what makes a schema page's cost finite: unbounded, one component's page carried 33 527 rows of columns and cost an hour and a half to build. Above this the page says how many entries it did not write and links the published schema, which carries all of them. Larger than the diagram's bound on purpose - a diagram of a hundred tables is already at the edge of what renders, and a list can afford far more than a picture |
