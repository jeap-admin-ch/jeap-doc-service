...
| `jeap.doc.custom.refused-extensions` | the list on [What an upload is validated against](upload-validation.md) | What an **HTML** microsite may not carry - executables, the scripts a workstation runs, and the archives that install them. A microsite follows no template, so there is no allowlist to bound it: what a build emits is carried, and what it may *do* is bounded by the sandbox it is served and framed with. Setting this replaces the list rather than adding to it. A Markdown set is bounded by its template's extensions and `additional-asset-extensions` instead |
| `jeap.doc.custom.additional-asset-extensions` | empty | Asset types a **Markdown** set may carry on top of its template's list - for example `[xlsx, mp4]`. It only adds: the template's list always applies, and an HTML microsite ignores this. Entries are lower-cased and lose a leading dot. An entry that can never be an asset fails the startup: `md` and `mdx`, what the site would render as a document (`html`, `htm`, `xhtml`, `shtml`), code (`js`, `mjs`, `cjs`, `jsx`, `ts`, `tsx`, `css`), and the executables of the default `refused-extensions`. The validation report's `allowedExtensions` lists what this instance takes |
  ...
  doc:
    sites:
      ...
      governance:
        ...
        publish-on-upload: false               # published by the import and by an operator only
...
| `publish-on-upload`           | `true`                                                          | Whether an upload for this site asks for a build of the part that carries the system it is for                                                                                                                                                                                                                                                                      |
...
| `jeap.doc.build.poll-interval`          | `PT30S`            | How often an **idle** instance looks whether a build has been asked for. It is not what decides how fast a queue of parts drains: a pass builds what is owed and reads again after every build, so this only bounds how long an instance that was told nothing takes to notice - see [Generation](generation.md) |
| `jeap.doc.build.pick-up-on-trigger`     | `true`             | Whether a trigger starts a build pass at once instead of leaving it to the next poll. The instance that took an upload knows there is work, and waiting a poll interval to look is latency for nothing. **Advisory either way**: a wake-up that is lost costs nothing, because the request stands and the next poll serves it |
| `jeap.doc.build.max-concurrent-parts`   | `3`                | How many parts of the documentation one instance builds at a time. **A memory decision rather than a parallelism one**: every concurrent build holds a Docusaurus run in the same container, and the memory of a build is what decides whether it survives - see `max-node-memory` below and [Observability](observability.md#the-memory-of-the-container). A full publication of a large landscape is a great many builds, and one after another they pay the fixed cost of a generator start each while the cores sit idle. Raise it against the memory a build is measured to hold, and not past the cores. It is how many run **at once** and not how many a pass builds: at `1` the parts are built one after another, within the same pass |
| `jeap.doc.build.timeout`                | `PT15M`            | How long a build may take before it is given up on                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `jeap.doc.build.lock-lease`             | `PT2M`             | How long a part's build lock is leased for - and therefore **how long after an instance dies its lock survives it**. Far shorter than a build may take, because the lock is extended while the build runs. The shortest accepted is 30 seconds, checked while the service starts                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `jeap.doc.build.ssg-task-size`          | `100`              | How many pages the site generator hands one static-generation worker at a time. Read only when `ssg-worker-threads` is on. **Not the generator's own default of ten**: at ten the workers wait on the one thread that hands the chunks out and collects their results - a worker is busy 66% of the time at four threads, 40% at eight - and a task of ten pages also caps how many pages one worker can have in flight. At a hundred the static generation of a large part is less than half of what it was. Above a hundred the tasks run out before the threads do, and the pool degenerates towards serial for the small parts of a site. **At least 1**: the generator does not check the value, and below one it renders no page at all and reports no error, so the service refuses to start |
| `jeap.doc.build.perf-log`               | `true`             | Whether the site generator reports how long each phase of a build took and what it did to the Node heap. The lines carry the prefix `[PERF]`, nested by phase, each heap reading taken after a full garbage collection - so a build that grows or slows down names the phase responsible. The collections cost a large site seconds. **Logged at `DEBUG`, like every other line of the generator's output**: a build writes hundreds of lines and a publication is dozens of builds, so at `INFO` the generator's own output would be most of what an instance logs. Left on by default so that the lines are there the moment `ch.admin.bit.jeap.doc.sitegenerator.NodeProcess` is turned down to `DEBUG`, without a restart |
| `jeap.doc.build.retention`              | `3`                | How many publications are kept **per part**. At least 2, checked while the service starts: the one being served, and the one other instances may still be serving from their publication cache                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `jeap.doc.build.history-retention`      | `P14D`             | How long the record of a build is kept. A part per system means a row per part per import - some eight hundred a day on a site of fifty-two parts, nearly all of them saying a part had not moved - so this is a fortnight rather than a quarter. **The published build of each part is kept whatever its age** - it is what says which publication a part is served from                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `jeap.doc.build.reconcile-cron`         | `0 15 6-18/4 * * *` | When to ask for the sites **no architecture import publishes** - one whose environments have no architecture repository. Every four hours through the working day; `-` switches it off. Such a site has no import to publish it, and the content digest covers the service version, so without this it keeps serving what an earlier release generated. Nearly free: a part whose content has not moved is not generated |
| `jeap.doc.build.departed-part-retention` | `P90D`            | How long a part the site no longer has keeps what it published. On the system axis a decommissioned system takes its part with it, and nothing else would ever remove its objects or its pages. Long on purpose, and the nightly sweep leaves a site alone entirely while its partition produces nothing but the shell - so a landscape that goes briefly wrong cannot take a live system's documentation with it. `DELETE /api/sites/{site}/parts/{part}` removes one without waiting |
| `jeap.doc.build.history-cron`           | `0 45 2 * * *`     | When old build records are removed, and when what a part the site no longer has published is removed, in the time zone of the service                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
One index over the whole site, built at the end of the build pass that published it - see
[Search](search.md). **There is no schedule**: a site is indexed when it changes.

| Property | Default | |
| -------- | ------- | --- |
| `jeap.doc.search.enabled`     | `true`              | Whether this instance indexes at all. **The way out when the service and the image do not ship together**: the indexer arrives with `node_modules`, so an instance whose image predates the search would otherwise fail its startup check. Off, and neither the check nor a build pass asks for it |
| `jeap.doc.search.retention`   | `2`                 | How many published indexes of a site are kept, the current one included. **Two is the minimum and it is refused below that**: every file the indexer writes is named by a content hash, so a reader whose browser holds the manifest of the index that was current a moment ago is still fetching chunks that exist only under its prefix |
...
jeap:
  doc:
    ...
    generator:
      ...
      max-detailed-edges: 20
      max-diagram-edges: 40
      max-edge-labels: 4
      max-context-components: 40
      max-schema-table-diagram: 100
      max-schema-table-list: 200
      rest-api-excluded-paths:
        - /actuator(/.*)?
      view-excluded-components: []
      view-excluded-relations: []
```
...
| `jeap.doc.archrepo.import.cache-landscape`                         | `true`            | Whether the landscape a build reads is held between builds instead of read from the database for each of them. A build reads the **whole** landscape of every environment its part carries, so a site of fifty parts over four environments read four landscapes two hundred times - about a third of a full publication. What is held is keyed on when that environment was last read successfully, so the import that could have changed a landscape is what drops it; a stale answer is not possible. It costs one landscape per environment in memory |
...
| `jeap.doc.generator.max-detailed-edges`                            | `20`              | How many relations a **whitebox** picture may draw in full - an arrow per kind and direction, in its colour, with the names on it. Above this the picture is folded to its shape: one grey line per pair of boxes, with an arrowhead only where the pair is joined one way. The relations table below it is unaffected and names every one |
| `jeap.doc.generator.max-diagram-edges`                             | `40`              | How many relations a **whitebox** picture may draw at all. Above this it is not drawn and the page says so in a sentence, because a whitebox view is dense rather than long-tailed: there is no part of it that is both readable and honest. Must not be below `max-detailed-edges`, which stops the startup |
...
| `jeap.doc.generator.max-context-components`                        | `40`              | How many **component boxes** a component's context view may draw - its siblings and the counterpart components of other systems together. Only the counterparts it actually exchanges something with are candidates. The siblings come first, then one component of each other system in turn, so one large neighbour cannot push a component's own siblings off its own page. Above the bound a system whose components got no box is drawn as a single box instead, the page says how many counterparts were left out, and its table of relations lists every one |
| `jeap.doc.generator.max-schema-table-diagram`                      | `100`             | How many entries an **entity relationship diagram** may draw, out of the ones the list below carries - so a box on the diagram always has a list entry, and the effective bound is the smaller of the two. The limit is the reader's browser rather than the page: the engine lays a diagram out by recursion, so a schema of three hundred tables is not a large picture but no picture at all. Above this the page says how many tables were left out, and the list of tables carries them as far as its own, larger bound below. **An entry is one table, or one family of partitions**: a partitioned table is grouped into a single entry before either bound applies                                   |
| `jeap.doc.generator.max-schema-table-list`                         | `200`             | How many entries a **database schema page** writes with their columns, by name. **The one bound on the facts rather than on a picture**, and what makes a schema page's cost finite: unbounded, one component's page carried 33 527 rows of columns and cost an hour and a half to build. Above this the page says how many entries it did not write. It links nothing: what carries all of them is the architecture repository's own API, which is not a page a reader of a published site can open. Larger than the diagram's bound on purpose - a diagram of a hundred tables is already at the edge of what renders, and a list can afford far more than a picture |
| `jeap.doc.generator.rest-api-excluded-paths`                        | `/actuator(/.*)?` | The paths a **REST API page** does not describe, as regular expressions that have to match a whole path - so `/health` means that one path and not `/api/health-reports`. **The default is the actuator**: every jEAP service publishes the operational endpoints the platform needs, they are in its specification, and on a small service they outnumber the operations a reader came for. A list rather than a rule about the actuator, because a component with a management context path of its own is the same question with a different answer. The page names the operations it left out, up to twenty, so the count can still be compared with the specification. An empty list documents every path. Applied when a page is written, so a change takes effect on the next build rather than on the next import; a pattern that is not a regular expression stops the startup |
| `jeap.doc.generator.view-excluded-components`                     | -                 | The components left out of the **diagrams and relations tables of other pages**, as regular expressions that have to match a whole component name - a test double that publishes the events of other systems, say. A system it alone exchanges something with is then no neighbour either. Its own pages stay, and say that it is left out. One list for the whole service. Applied when a page is written; a pattern that is not a regular expression stops the startup |
| `jeap.doc.generator.view-excluded-relations`                       | -                 | The **relations** left out of the diagrams and relations tables of other pages - the platform's plumbing rather than a component's architecture, such as every component uploading its schemas to the architecture repository. An entry is a set of fields (`consumer`, `provider`, `method`, `path`, `message-type`), **all of which have to match**; one that is not given matches anything. It removes arrows and never boxes: a component named here keeps its box and its pages, and its own page says that some of its relations are elsewhere left out. A `consumer`, a `provider` and a `message-type` are regular expressions over the whole name; a **`path` is not** - it is compared literally after normalising its variable names, because `/api/openapi/{systemComponentName}` is no regular expression and is what an operator writes. An entry with no field at all, or one naming a `path` or a `method` together with a `message-type`, stops the startup |
...
| Property                       | Default | Description                                                                                                                                                    |
|--------------------------------|---------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `jeap.doc.publication.url`     | -       | The origin the documentation is published under, without a path. It is what the page metadata names - the canonical URL of a page, and its Open Graph tags     |
...
**A read replica is used where the instance has one** (`jeap.datasource.replica.enabled`, from the PostgreSQL AWS
starter). Only the reads behind what is shown and served go there: the published site, the site status, the parts
listing, the live status and the import states. They may lag a moment. Every read that decides a write - a build,
an import, an upload, a clean-up - stays on the primary. Without a replica everything runs on the primary.
...
| `server.compression.*`                        | on, from 1KB                              | The documentation is text throughout - the HTML and the bundles. An instance behind a compressing CDN can turn it off                                                                                                                                         |
