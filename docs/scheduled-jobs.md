...
Everything the doc service does on its own, without a request: nine jobs, each with its schedule in the
...
| [Architecture import](architecture-import.md), per environment | `jeap.doc.archrepo.import.cron`            | `0 45 5-19 * * *` | Imports the architecture model, the OpenAPI specifications, the database schemas and the Avro schemas of the message type versions of one environment - and its reaction graphs where a [reaction observer](architecture-import.md) is configured for it - and **asks for every part of every site documenting it to be published**. Hourly. There is no separate publication schedule: this is it. **Empty means never**      |
...
| [Unreferenced object sweep](custom-documentation.md)           | `jeap.doc.custom.sweep-cron`               | `0 50 2 * * *`    | Removes the objects under the current documentation that **no documentation set names** - what a removal could not delete, and what an instance left behind by dying between copying a set's object and committing the rows that name it. It selects on references and never on age: nothing here may be removed for being old. An object written in the last six hours is left alone, because it may belong to an upload that is still committing |
...
| [Departed part sweep](generation.md)                           | `jeap.doc.build.history-cron`              | `0 45 2 * * *`    | Removes what a part **the site no longer has** is still publishing - a system that was decommissioned, so nothing asks for its part and nothing would ever take its pages down. Slow and guarded twice: only after `jeap.doc.build.departed-part-retention` (`P90D`) of publishing nothing, and never on a site whose partition produces nothing but the shell. A system that is only *documented* still has a part, so this does not reach it |
