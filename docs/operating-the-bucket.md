...
| `current/` | The documentation sets being published: one object per markdown set, copied from the upload that brought it, and one object **per file** under a `files/` prefix for an HTML microsite, which is served file by file | The service, and only when somebody removes a set or a subject. **Nothing here may be expired by age** - see below. A nightly sweep removes objects that no set *references*, which is a different question from how old they are |
