...
/**
 * The relations the systems of one environment define.
 */
interface ArchitectureRelationJpaRepository extends JpaRepository<ArchitectureRelationEntity, Long> {
     ...
     */
    @Query(value = "select * from architecture_relation where system_id = any(:systemIds) order by ordinal",
            nativeQuery = true)
    List<ArchitectureRelationEntity> findBySystemIdInOrderByOrdinal(@Param("systemIds") Long[] systemIds);
}
