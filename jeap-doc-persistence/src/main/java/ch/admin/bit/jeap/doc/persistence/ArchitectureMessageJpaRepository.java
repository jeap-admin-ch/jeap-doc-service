...
/**
 * The events and commands the systems of one environment define.
 */
interface ArchitectureMessageJpaRepository extends JpaRepository<ArchitectureMessageEntity, Long> {
     ...
     */
    @Query(value = "select * from architecture_message where system_id = any(:systemIds) order by ordinal",
            nativeQuery = true)
    List<ArchitectureMessageEntity> findBySystemIdInOrderByOrdinal(@Param("systemIds") Long[] systemIds);
}
