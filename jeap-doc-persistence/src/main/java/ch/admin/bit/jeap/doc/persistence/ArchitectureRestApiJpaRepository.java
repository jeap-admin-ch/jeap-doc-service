...
/**
 * The REST operations the components provide.
 */
interface ArchitectureRestApiJpaRepository
        extends JpaRepository<ArchitectureRestApiEntity, ArchitectureRestApiEntity.Key> {
     ...
     */
    @Query(value = "select * from architecture_rest_api where component_id = any(:componentIds) order by ordinal",
            nativeQuery = true)
    List<ArchitectureRestApiEntity> findByComponentIdInOrderByOrdinal(@Param("componentIds") Long[] componentIds);
}
