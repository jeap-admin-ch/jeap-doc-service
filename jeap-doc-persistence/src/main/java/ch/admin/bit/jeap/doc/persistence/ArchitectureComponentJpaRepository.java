...
/**
 * The components of the systems of one environment.
 */
interface ArchitectureComponentJpaRepository extends JpaRepository<ArchitectureComponentEntity, Long> {
     ...
     */
    @Query(value = "select * from architecture_component where system_id = any(:systemIds) order by ordinal",
            nativeQuery = true)
    List<ArchitectureComponentEntity> findBySystemIdInOrderByOrdinal(@Param("systemIds") Long[] systemIds);
}
