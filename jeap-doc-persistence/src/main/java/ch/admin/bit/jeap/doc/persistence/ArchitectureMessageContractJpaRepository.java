...
/**
 * Who produces and who consumes the messages.
 */
interface ArchitectureMessageContractJpaRepository extends JpaRepository<ArchitectureMessageContractEntity, Long> {
     ...
     */
    @Query(value = "select * from architecture_message_contract where message_id = any(:messageIds) order by ordinal",
            nativeQuery = true)
    List<ArchitectureMessageContractEntity> findByMessageIdInOrderByOrdinal(@Param("messageIds") Long[] messageIds);
}
